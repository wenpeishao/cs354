// --== CS354 Fall 2022 File Header Information ==--
// Name: <Wenpei Shao>
// Net ID: wshao33
// CSL Login: wenpei
...
void access_data(mem_addr_t addr) {      
	...
	miss_cnt++;//miss
	for(int i = 0; i < E; i++){
       		if(!(cache[set][i].valid)){ // not valid
			cache[set][i].tag = tag; 
			cache[set][i].lru = lru_counter; 
			cache[set][i].valid = 1;
            lru_counter++;
    ...
    cache[set][change_line].lru = lru_counter; 
    cache[set][change_line].tag = tag;
...
void replay_trace(char* trace_fn) {           
    ...
    while (fgets(buf, 1000, trace_fp) != NULL) {
	    ...
	    if(buf[1] == 'S'){//when buf[1] is S
		    access_data(addr);//access 1 time
	    }
	    else if(buf[1] == 'L'){ //when buf[1] is L
		    access_data(addr); //access 1 time
	    }
	    else if(buf[1] == 'M'){ //when buf[1] is M
		    access_data(addr);
		    access_data(addr);//access twice
	    }
	    if (verbosity){
    		    printf("\n");
	    }
